...
const os = require('os');
const { selectPreferredNetworkAddress } = require('./server/network-address');
...
serverReady.then((port) => {
  const prefix = process.env.EASYLOOP_LAN_PREFIX || '192.168.30.';
  const address = selectPreferredNetworkAddress(os.networkInterfaces(), prefix);
